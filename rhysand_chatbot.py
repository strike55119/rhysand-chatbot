""" Rhysand Chatbot — Web App

A simple web-based chatbot that lets you chat with Rhysand, High Lord of the Night Court, using OpenAI’s chat completions API.

Run this file with a web framework like Flask. """

from flask import Flask, request, render_template_string import openai import os
...
openai.api_key = os.getenv("OPENAI_API_KEY") if not openai.api_key: raise RuntimeError("Please set the OPENAI_API_KEY environment variable.")
...
"""@app.route("/", methods=["GET", "POST"]) def chat(): if request.method == "POST": user_input = request.form.get("user_input", "") if user_input.strip(): chat_history.append({"role": "user", "content": user_input})
